...
class GameServer {
    ...
    private val blockedCells2 = setOf(
        GridPos(-2, 2),
        GridPos(1, 2),
        GridPos(2, 2),
        GridPos(2, 1)
    ...
    private fun isCellBlocked2(x: Int, z: Int): Boolean{
        // проверка, запрещена ли клетка для входа в нее
        return GridPos(x, z) in blockedCells2
    }
    ...
    private suspend fun processCommand(cmd:  GameCommand){
        when(cmd){
            is CmdStepMove -> {
                ...
                // Общая функция проверки клетки (объединяет оба набора)
                fun isBlocked(x: Int, z: Int): Boolean {
                    return !isCellInsideMap(x, z) ||
                            GridPos(x, z) in blockedCells ||
                            GridPos(x, z) in blockedCells2
                }

                val isDoubleStep = kotlin.math.abs(cmd.stepX) == 2 || kotlin.math.abs(cmd.stepZ) == 2

                if (isDoubleStep) {
                    val midX = player.gridX + (cmd.stepX / 2)
                    val midZ = player.gridZ + (cmd.stepZ / 2)

                    if (isBlocked(midX, midZ) || isBlocked(targetX, targetZ)) {
                        _events.emit(ServerMessage(cmd.playerId, "Путь заблокирован! Нельзя перешагнуть через препятствие"))
            ...
            is CmdInteract -> {
                ...
                if (obj == null){
                    _events.emit(ServerMessage(cmd.playerId, "Нет объектов для взаимодействия рядом"))
                    return
                }
                ...
                when (obj.type){
                    WorldObjectType.ALCHEMIST -> {
                        if (player.alchemistMemory.sawPlayerNearSource){
                            _events.emit(ServerMessage(cmd.playerId, "Ты траву забыл"))
                            return
                        }else{
